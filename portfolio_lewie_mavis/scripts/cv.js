...
function initProfileBadges() {
    ...
    profileBadges.forEach((badge, index) => {
        ...
        const observer = new IntersectionObserver((entries) => {
            entries.forEach(entry => {
                if (entry.isIntersecting) {
                    badge.classList.add('bounce-in-active');
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function initializeTechSphere() {
        ...
        const techTags = [
            ...
            'AWS', 'Docker', 'Kubernetes', 'AI/ML', 'Cybersecurity',
            'Blockchain', 'IoT', 'DevOps', 'Cloud', 'Full Stack'
        ];

        const colors = ['#00f7ff', '#64ffda', '#4CAF50', '#2196F3', '#9C27B0'];
        const sphereContainer = document.createElement('div');
        sphereContainer.className = 'sphere-container';
        ...
        techTags.forEach((tag, index) => {
            ...
            tagElement.className = 'tech-tag-sphere';
            tagElement.textContent = tag;
            tagElement.style.color = colors[index % colors.length];
            
            // Position tags in 3D space
            const phi = Math.acos(-1 + (2 * index) / techTags.length);
        ...
        document.addEventListener('mousemove', (e) => {
            ...
            sphereContainer.style.transform = `rotateY(${rotationY}deg)`;
            previousMouseX = e.clientX;
        });
        ...
        // Add touch support
        techSphere.addEventListener('touchstart', (e) => {
            isDragging = true;
            previousMouseX = e.touches[0].clientX;
        });

        techSphere.addEventListener('touchmove', (e) => {
            if (!isDragging) return;
        ...
        techSphere.addEventListener('touchend', () => {
            isDragging = false;
        });
    }

    // Download CV PDF functionality
    ...
    if (downloadBtn) {
        downloadBtn.addEventListener('click', function(e) {
            ...
            setTimeout(() => {
                // In a real implementation, this would generate/retrieve the PDF
                alert('CV PDF download would start here. In production, this would link to your actual PDF file.');
    ...
    // CV Contact Form
    const cvContactForm = document.getElementById('cv-contact-form');
    if (cvContactForm) {
        cvContactForm.addEventListener('submit', function(e) {
            e.preventDefault();
    ...
    function animateOnScroll() {
        ...
        const observer = new IntersectionObserver((entries) => {
            entries.forEach(entry => {
                if (entry.isIntersecting) {
                    const animationType = Array.from(entry.target.classList)
                        .find(cls => cls.includes('slide') || cls.includes('fade') || cls.includes('zoom') || cls.includes('blur') || cls.includes('flip') || cls.includes('rotate') || cls.includes('bounce'));
...
function initializeCVDownload() {
    ...
    if (downloadBtn) {
        downloadBtn.addEventListener('click', async function(e) {
            ...
            button.innerHTML = '<i class="fas fa-spinner"></i><span>Preparing CV</span>';
            ...
            try {
                ...
                button.innerHTML = '<i class="fas fa-check"></i><span>CV Downloaded!</span>';
                ...
                // Show error state
                button.classList.remove('loading');
                button.innerHTML = '<i class="fas fa-exclamation-triangle"></i><span>Download Failed</span>';
...
async function downloadCVAsPDF() {
    // In production, this would be your actual PDF file
    const pdfUrl = 'path/to/your/cv.pdf'; // Replace with your actual PDF path
    
    // Create a temporary download link
    const link = document.createElement('a');
    link.href = pdfUrl;
    link.download = 'Lewie_Mavis_CV.pdf';
    link.target = '_blank';
    ...
    // If you're generating PDF dynamically, you could use a library like jsPDF
    // Here's an example of how you might implement dynamic PDF generation:
    /*
    if (typeof jsPDF !== 'undefined') {
        const { jsPDF } = window.jspdf;
        const doc = new jsPDF();
        
        // Add your CV content to the PDF
        doc.setFontSize(22);
        doc.text('LEWIE MAVIS', 105, 30, null, null, 'center');
        doc.setFontSize(16);
        doc.text('Computer Scientist & Software Solutions Expert', 105, 40, null, null, 'center');
        
        // Add more sections...
        
        // Save the PDF
        doc.save('Lewie_Mavis_CV.pdf');
    }
    */
...
function initializePrintCV() {
    ...
    if (!document.querySelector('.print-cv-btn')) {
        const printBtn = document.createElement('button');
        printBtn.className = 'print-cv-btn animate zoom-in';
        printBtn.innerHTML = '<i class="fas fa-print"></i><span>Print CV</span>';
        ...
        // Animate print button in
        setTimeout(() => {
            printBtn.classList.add('zoom-in-active');
        }, 1000);
    }
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Add keyboard shortcut for download (Ctrl+D or Cmd+D)
    ...
    // Show download hint on hover
    const downloadBtn = document.getElementById('download-cv-pdf');
    if (downloadBtn) {
        downloadBtn.title = "Click to download CV (PDF)\nShortcut: Ctrl+D";
    }
});

// Optional: If you want to actually generate a PDF with jsPDF, add this script to your HTML head:
/*
